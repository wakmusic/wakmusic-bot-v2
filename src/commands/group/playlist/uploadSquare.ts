...
const playlistUploadSquareCommand: SubCommand = {
  name: CommandName.PLAYLIST_SUB_UPLOAD_SQUARE,
  execute: async (interaction: ChatInputCommandInteraction): Promise<void> => {
    ...
    const url = await fileService.uploadImage({
      path: 'static/playlist/icon/square',
      fileName: playlist.key,
      version: playlist.image.square + 1,
      fileType: 'png',
    ...
    try {
      await playlistService.updateImage(playlist.image.id, {
        square: playlist.image.square + 1,
      });
    } catch (error) {
      logger.error(error);
...
export { playlistUploadSquareCommand };
